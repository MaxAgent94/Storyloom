...
Supabase stores each project's editable body separately from its chat/proposal history, within the same owner-protected row. Apply `database/migrations/20260919071227_separate_project_history.sql` after the initial schema (also for existing installations). `save_project_v2` accepts incremental collection changes, checks the revision under a row lock, and calls the existing transactional section-versioning function. Unchanged history is neither fetched nor rewritten during routine manuscript saves. Existing documents convert on their next successful save; older full-document writers remain readable. No content or historical versions are pruned. API reads reassemble the portable project format, so exports and existing clients remain compatible. Gzip transport limits incoming compressed bodies to 3.5 MB and decompressed bodies to 64 MB; routine edits send changed records only. Same-field manual manuscript conflicts preserve the author's current edit while retaining saved section versions.
